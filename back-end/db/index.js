...
// Products and Product items functions
const getProducts = async (category_id=undefined, search_term=undefined) => {
  const baseQuery = 'SELECT id, name, price, available_slot_count, short_description, long_description, avg_return, return_count FROM products';
  let res;
  if (category_id) {
    res = await query(
...
const getCartItems = async (user_id) => {
  ...
  const join = 'JOIN products ON cart_items.product_id = products.id';
...
const cartItemExists = async (user_id, product_id) => {
  res = await query(
    'SELECT user_id, product_id FROM cart_items WHERE user_id=$1 AND product_id=$2',
    [user_id, product_id]
  );
  return res.rowCount > 0;
};
...
const addCartItem = async (user_id, product_id, product_quantity=1) => {
  const insert = 'INSERT INTO cart_items(user_id, product_id, quantity) VALUES($1, $2, $3)';
  const update = 'UPDATE products SET available_slot_count = (available_slot_count - $3) WHERE id=$2 RETURNING name, price';
  const res = await query(
...
const deleteCartItem = async (user_id, product_id) => {
  const deleteRes = await query(
    'DELETE FROM cart_items WHERE user_id=$1 AND product_id=$2 RETURNING quantity',
    [user_id, product_id]
  );
  try {
    ...
    await query(
      'UPDATE products SET available_slot_count = (available_slot_count + $1) WHERE id=$2',
      [quantity, product_id]
    );
  } catch(err) {
    console.log(err);
...
const getAddressById = async (id) => {
  const res = await query('SELECT address, postcode, phone FROM contact_details WHERE id=$1', [id]);
  return res.rows[0];
};

const getAddressId = async (address, postcode, phone) => {
  const res = await query(
    'SELECT id FROM contact_details WHERE address=$1 AND postcode=$2 AND phone=$3',
    [address, postcode, phone] 
  );
  return res.rows.length === 1 ? res.rows[0].id : undefined;
};

const addAddress = async (address, postcode, phone) => {
  const res = await query(
    'INSERT INTO contact_details(address, postcode, phone) VALUES($1, $2, $3) RETURNING id',
    [address, postcode, phone] 
  );
...
const createPendingOrder = async (user_id, address_id) => {
  ...
  try {
    ...
    const addressRes = await client.query(
      'SELECT address, postcode FROM contact_details WHERE id=$1',
      [address_id]
    );
...
const confirmPaidOrder = async (order_id) => {
  ...
  try {
    ...
    for await (const product of order.order_items) {
      ...
      await client.query(
        'UPDATE products SET slot_count = (slot_count - $1) WHERE id=$2',
...
const getOrderById = async (id) => {
  const orderSelect = 'SELECT orders.id, user_id, order_placed_time, status, total_cost, address, postcode, phone';
  const addressesJoin = 'JOIN contact_details ON orders.address_id = contact_details.id';
  ...
  const productsJoin = 'JOIN products ON order_items.product_id = products.id'
  const orderItemsRes = await query(
    `${orderItemsSelect} FROM order_items ${productsJoin} WHERE order_id=$1`,
    [id]
  );
