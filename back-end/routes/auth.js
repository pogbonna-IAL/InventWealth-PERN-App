...
router.get('/register', jsonParser, (req, res) => {
  res.status(404).send('Please enter a valid user request to register.');
});
  ...
  try {
    ...
    if (userExists) {
      return res.status(400).send(
        `Registration failed. The email '${email_address}' is already in use; please use another.`);
    }
...
router.get('/login', jsonParser, (req, res) => {
  res.status(404).send('Please make a valid user request to log in.');
});
