...
router.post('/create-pending-order', requireLogin, jsonParser, async (req, res) => {
  try {
    ...
    const { address, postcode, phone } = req.body;
    if (!(address && postcode && phone)) {
      return res.status(400).send('Please provide a valid address, postcode and phone in the request body.');
    }
    ...
    let addressId = await db.getAddressId(address, postcode, phone);
    if (!addressId) {
      addressId = await db.addAddress(address, postcode, phone);
    }
    ...
    const orderItemsData = orderData.order_items.map(item => {
      return {
        price_data: {
          ...
          unit_amount: (Number(item.product_price.substring(1)) * 100),  // Price in pence
        },
