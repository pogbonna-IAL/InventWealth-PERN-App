...
router.param('id', (req, res, next, id) => {
  if (!req.isAuthenticated() || id !== String(req.user.id)) {
    return res.status(401).send(
      `Invalid login details. You must be logged in as the user with id '${id}'.`
    );
  }
  next();
});
...
router.put('/:id', jsonParser, async (req, res) => {
  try {
    ...
    res.status(500).send(
      'Password update failed. Please ensure you are providing the required data that is valid.'
    );
  }
});
