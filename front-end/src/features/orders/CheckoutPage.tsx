  ...
  try {
    ...
    const res = await fetch(
      `${process.env.REACT_APP_API_BASE_URL}/checkout/create-pending-order`,
      {
        ...
        credentials: "include",
        body: JSON.stringify({ address, postcode, phone })
      }
    );
  ...
  return (
    <div className={utilStyles.pagePadding}>
      ...
      <Form method="post" className={utilStyles.stackedForm}>
        ...
        <label htmlFor="phone" className={utilStyles.label}>Phone</label>
        <input id="phone" className={utilStyles.input} type="text" name="phone" minLength={10} maxLength={12} required />
