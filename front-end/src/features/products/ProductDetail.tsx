...
export async function addToCartAction({ params }) {
  ...
  try {
    ...
    return "Sorry, this item could not be added to your cart.";
  }
}
...
export function ProductDetail() {
  ...
  const { short_description, long_description, avg_return, return_count, price } = productData;
  const slot_count = productData.available_slot_count;
  ...
  function renderButton() {
    ...
    if (slot_count < 1) {
      return <p className={utilStyles.largeText}><em>Out of slots</em></p>;
    ...
    <div className={utilStyles.pagePadding}>
      <section className={styles.summarySection}>
        ...
        <div className={styles.summaryTextContent}>
          ...
          <p className={styles.price}>{"NGN"+" "+price+"M"}</p>
          <hr />
          <p>{short_description}</p>
          <hr />
          {(slot_count >= 1 && slot_count <= 20) ? <p><strong>Only {slot_count} left in slot!</strong></p> : null }
          {renderButton()}
          {addToCartMessage ? <p className={styles.buttonMessage}>{addToCartMessage}</p> : null}
          {avg_return ?
          <div>
            <StarRating rating={avg_return} />
            <div className={styles.returnText}>Rated {avg_return}/100.00 based on {return_count} ratings</div>
          </div>
          : null}
        </div>
      </section>
